...
export const DoctorManager: React.FC = () => {
  ...
  const fetchDoctorSchedule = async (doctorId: string) => {
    try {
      const { data } = await apiClient.get(`/academic/doctors/${doctorId}/schedule`)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (r: Doctor) => {
        const max = r.max_hours_per_week || 16
    ...
    },
    {
      ...
      render: (r: Doctor) => (
        <div className="flex gap-2">
          <button onClick={() => { setSelectedDoctor(r); fetchDoctorSchedule(r.id); setShowScheduleModal(true); }}
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#020817] flex items-center justify-center">
        <div className="text-[#94A3B8]">Loading doctors...</div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#020817] flex">
        ...
        <div className="p-6 lg:p-8">
          <motion.div variants={fadeInUp} initial="hidden" animate="visible" className="flex items-center justify-between mb-8">
            <div>
              ...
              <h1 className="font-sora text-3xl font-bold text-[#F8FAFC]">Doctor <span className="gradient-gold">Manager</span></h1>
            </div>
            <GlowButton variant="primary" size="md" magnetic icon={<Plus className="w-4 h-4" />} onClick={() => setShowModal(true)}>Add Doctor</GlowButton>
          </motion.div>
            ...
            <motion.div variants={staggerItem}>
              <GlassCard className="p-5 hover">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm text-[#94A3B8]">Total Doctors</span>
                </div>
                <p className="text-3xl font-bold font-sora text-[#F8FAFC]">{doctors.length}</p>
              </GlassCard>
            </motion.div>
            <motion.div variants={staggerItem}>
              <GlassCard className="p-5 hover">
                ...
                <p className="text-3xl font-bold font-sora text-[#F8FAFC]">{doctors.reduce((a, d) => a + d.current_hours, 0)}</p>
              </GlassCard>
            </motion.div>
            <motion.div variants={staggerItem}>
              <GlassCard className="p-5 hover">
                ...
                <p className="text-3xl font-bold font-sora text-[#F8FAFC]">
                  {doctors.length > 0 ? `${Math.round(doctors.reduce((a, d) => a + (d.max_hours_per_week > 0 ? (d.current_hours / d.max_hours_per_week) * 100 : 0), 0) / doctors.length)}%` : '0%'}
                </p>
              </GlassCard>
            </motion.div>
          </motion.div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Add New Doctor" size="md">
        <div className="space-y-4">
          <GlowInput label="Full Name" placeholder="Dr. Ahmed Hassan" value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} />
          <GlowInput label="University Email" type="email" placeholder="doctor@hitu.edu.eg" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} />
          ...
          <div className="flex gap-3 pt-2">
            ...
            <GlowButton variant="primary" fullWidth magnetic onClick={handleCreate}>Add Doctor</GlowButton>
          </div>
        </div>
      </Modal>

      <Modal open={showScheduleModal} onClose={() => setShowScheduleModal(false)} title={`${selectedDoctor?.full_name} Schedule`} size="lg">
        {doctorSchedule && (
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              <GlassCard className="p-4">
                <p className="text-xs text-[#94A3B8] mb-1">Assigned Hours</p>
                <p className="text-2xl font-bold font-sora text-[#F8FAFC]">{doctorSchedule.total_assigned_hours}/{doctorSchedule.max_hours}h</p>
              </GlassCard>
              <GlassCard className="p-4">
                <p className="text-xs text-[#94A3B8] mb-1">Courses Assigned</p>
                <p className="text-2xl font-bold font-sora text-[#F8FAFC]">{doctorSchedule.courses_assigned}</p>
              </GlassCard>
              <GlassCard className="p-4">
                <p className="text-xs text-[#94A3B8] mb-1">Schedule Entries</p>
                <p className="text-2xl font-bold font-sora text-[#F8FAFC]">{doctorSchedule.schedule_entries.length}</p>
              </GlassCard>
            </div>
            <GlassCard className="p-4">
              <p className="text-sm text-[#94A3B8] mb-3">Weekly Schedule</p>
              {doctorSchedule.schedule_entries.length > 0 ? (
                <div className="space-y-2">
                  {doctorSchedule.schedule_entries.map((entry, idx) => (
